...
public class IdentifyService {
    ...
    public List<Circle> countCSVCircle(String workspaceId, String params) {
        String queryString = "select c.* from circle c where c.workspaceid = :workspaceId and c.isdefault = false and c.csv = true " +
                "and c.enabled = true and jsonb_exists_any(c.parameters,array['" + params + "']) limit 1";
    ...
    public Circle findCSVCircleDefaultByWorkspaceIdAndValue(String workspaceId, String key, String value) {
        String defaultQueryString = "select c.* from circle c where c.workspaceid = :workspaceId and c.enabled = true " +
                "and c.isdefault = false and c.csv = true and c.match->>'" + key + "' = '" + value + "'" ;
    ...
    public List<Circle> findCircleByWorkspaceIdAndParams(String workspaceId, String params) {
        String queryString = "select c.* from circle c where c.workspaceid = :workspaceId and c.isdefault = false and c.csv = false and c.enabled = true and jsonb_exists_any(c.parameters,array[" + params + "])";
        Query query = getEntityManager().createNativeQuery(queryString, Circle.class);
        query.setParameter("workspaceId", workspaceId);
